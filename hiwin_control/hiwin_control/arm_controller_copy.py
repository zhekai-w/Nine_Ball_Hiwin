...
def apply_rotation(pose, R):
    # Convert the pose's angular values into a vector
    angles = np.array([pose.angular.x, pose.angular.y, pose.angular.z])
    # Apply rotation matrix R
    new_angles = np.dot(R, angles)
    # Update pose's angular values
    pose.angular.x, pose.angular.y, pose.angular.z = new_angles
    return pose
