    ...
    def _state_machine(self, state: States) -> States:
        ...
        #開始做動態校正
            ...
            if self.index < len(self.target_cue): #初始index=0;target_cue=>2*2矩正
                ...
                #     # mid_x, mid_y = self.target_cue[self.index]
                ...
                #fix_z目标的固定距离（z轴上的距离）
                # tool_to_cam[2]：工具到相机的距离在z轴上的分量。由于是绝对值，它可能表示相机在工具坐标系中的位置偏移。
                #self.table_z：桌子的z轴高度或偏移量。可能是为了调整目标在实际工作环境中的高度。
                #self.target_cue[self.index]：这是当前目标球的像素坐标。它提供了目标在相机图像中的位置。
                ball_relative_cam = pixel_mm_convert(self.fix_z - abs(tool_to_cam[2]) + abs(self.table_z), self.target_cue[self.index])
                req = self.generate_robot_request(cmd_mode=RobotCommand.Request.CHECK_POSE)
                res = self.call_hiwin(req)
                ...
                if res.arm_state == RobotCommand.Response.IDLE and self.index < len(self.target_cue):
                    ...
                    # 动态校正完成后打印更新的目标位置
                # print("Updated Target Cue:", self.updated_target_cue)

                # # 击球前打印目标位置
                # print("Preparing to hit ball at position:", self.updated_target_cue)
         ...
         #決策是用手臂坐標系      
            ...
            #剛剛更新的球點位    # 先目標球在白球
            # #updated_target_cue[目標球0 目標球1 白球-2 白球-1] 
            # #actual_x[目標球0 白球-1] 
            # #actual_y[目標球0 白球-1]
            ...
            cuex, cuey = self.updated_target_cue[-2], self.updated_target_cue[-1] #[白球 白球]
            ...
            self.strategy_info = table2.main(actual_x[:-1], actual_y[:-1], cuex, cuey) #[白球 白球] #[白球 白球]
