...
bool detect_interface__msg__detection_results__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("detect_interface.msg._detection_results.DetectionResults", full_classname_dest, 56) == 0);
  }
  detect_interface__msg__DetectionResults * ros_message = _ros_message;
  {  // labels
  ...
  {  // bounding_boxes
    ...
    if (!detect_interface__msg__BoundingBox__Sequence__init(&(ros_message->bounding_boxes), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create detect_interface__msg__BoundingBox__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    detect_interface__msg__BoundingBox * dest = ros_message->bounding_boxes.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!detect_interface__msg__bounding_box__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of DetectionResults */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("detect_interface.msg._detection_results");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "DetectionResults");
  ...
  {  // bounding_boxes
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = detect_interface__msg__bounding_box__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
